...
min_points_per_plane = 100 # small plane filter threshold
...
save_image = True # running with image compute and save image
...
path_surround = pathlib.Path(pcd_path_surround)
pcd = o3d.io.read_point_cloud(str(path))
pcd_surround = o3d.io.read_point_cloud(str(path_surround))
pcd_complet = pcd+pcd_surround
...
pcd.scale(pcd_scaler(pcd), pcd_complet.get_center())
pcd.paint_uniform_color([0.6, 0.6, 0.6])
pcd_complet.scale(pcd_scaler(pcd_complet), pcd_complet.get_center())
pcd_complet.paint_uniform_color([0.4, 0.4, 0.4])
# dists = pcd.compute_nearest_neighbor_distance()
# avg_d  = np.mean(dists)
# if avg_d > 0.5:
#     scale_factor = 1/1000
...
# for iii in range(2,3):
    ...
    points_between_p2,points_beside = select_points_between_planes(pcd_complet, center_i, center_j, plane_normals[mmm],margin=margin_points_between_planes)
    ...
    if no_image == False:
        if show_each_P_in_pcd == True: 
            o3d.visualization.draw_geometries([pcd_complet, overlap_pcd.translate([0,0,0.0001])],window_name="PCD+P1")
            o3d.visualization.draw_geometries([pcd_complet, proj_pcd_p2.translate([0,0,0.0001])],window_name="PCD+P2")
            o3d.visualization.draw_geometries([pcd_complet, proj_pcd_p3.translate([0,0,-0.0001])],window_name="PCD+P3")
            o3d.visualization.draw_geometries([pcd_complet, proj_pcd_p4.translate([0,0,-0.0001])],window_name="PCD+P4")
            o3d.visualization.draw_geometries([pcd_complet, proj_pcd_p5.translate([0,0,-0.0001])],window_name="PCD+P5")
    ...
    def rank_feasible_tcp(feasible_TCP_2d, intersection_areas,w1,w2):
        # 2D mean of pcd_complet
        pcd_complet_points = np.asarray(pcd_complet.points)
        means = np.mean(pcd_complet_points, axis=0)
        # find the farest distance to mean point within the whole pcd
        distances = np.linalg.norm(pcd_complet_points - means, axis=1)
